    ...
    for model_name, model_class in zip(model_names, model_classes):
        ...
        if model_names == ['wan_video_dit']:
            with init_weights_on_device():
                # Enable ControlNeXt by default
                model = model_class(control_layers, use_controlnext=True, **extra_kwargs)
            if hasattr(model, "eval"):
                model = model.eval()
            # Load only main model weights; ControlNeXt injectors are newly initialized and lightweight
            model.load_state_dict(model_state_dict, assign=True, strict=False)
            # Use to_empty() for models initialized on meta device
            model = model.to_empty(device=device)
            # Convert to target dtype and reload state dict after moving to target device
